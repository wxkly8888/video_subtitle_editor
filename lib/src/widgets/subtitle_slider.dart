...
class _SubtitleSliderState extends State<SubtitleSlider> {
  ...
  videoUpdate() {
    ...
    if (widget.controller.video.value.isPlaying) {
      ...
      int interval = widget.controller.videoPosition.inMilliseconds-lastTimeStamp;
      lastTimeStamp = widget.controller.videoPosition.inMilliseconds;
      if(interval>0){
        _scrollController.animateTo(speed*lastTimeStamp, duration: Duration(milliseconds: interval), curve: Curves.linear);
      }
      // _scrollController.position.correctBy();
    }
  }
