...
import 'package:video_subtitle_editor/src/utils/subtitle/types.dart';
...
class SubtitleParser extends ISubtitleParser {
  ...
  List<Subtitle> parsing({
    ...
    var matches = regex.allMatches(object.data);
  ...
  List<Subtitle> _decodeSubtitleFormat(
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      final data = shouldNormalizeText
          ? normalize(matcher.group(10)?.trim() ?? '')
  ...
  Duration _getStartDuration(Match matcher) {
    var minutes = 0;
    var hours = 0;
  ...
  Duration _getEndDuration(Match matcher) {
    var minutes = 0;
    var hours = 0;
...
typedef OnParsingSubtitle = List<Subtitle> Function(Iterable<Match> matchers);
...
class CustomSubtitleParser extends ISubtitleParser {
  ...
  @override
  List<Subtitle> parsing() {
    var matches = pattern.allMatches(object.data);
